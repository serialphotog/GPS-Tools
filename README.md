...
This installs the `csv2gpx` and `gpx2csv` console scripts.
...
- `--format`, `-f`: Allows you to specify a custom column mapping to use when generating the CSV file. See the [CSV Column Mapping](#csv-column-mapping) section for more information.
- `--force`: Overwrites the output file if it already exists.
...
Column mapping values are zero-based indexes. For example, `lat:0` maps the latitude to the first CSV column.
